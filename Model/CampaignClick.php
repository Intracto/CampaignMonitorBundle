...
/**
 * A subscriber who clicked a link in a given campaign,
 * including the date/time and IP address from which they clicked the link.
 *
 ...
 */
class CampaignClick extends CampaignResult
{
    /**
     * @var \DateTime
     */
    ...
    {
        return array_merge(parent::getRequiredFields(), ['Date', 'IPAddress', 'URL']);
    }
    ...
    public static function createFromResultArray(array $rawData)
    {
        ...
        if (isset($rawData['Latitude'])) {
            $campaignClick->setLatitude($rawData['Latitude']);
        }

        if (isset($rawData['Longitude'])) {
            $campaignClick->setLongitude($rawData['Longitude']);
        }

        if (isset($rawData['City'])) {
            $campaignClick->setCity($rawData['City']);
        }

        if (isset($rawData['Region'])) {
            $campaignClick->setRegion($rawData['Region']);
        }

        if (isset($rawData['CountryCode'])) {
            $campaignClick->setCountryCode($rawData['CountryCode']);
        }

        if (isset($rawData['CountryName'])) {
            $campaignClick->setCountryName($rawData['CountryName']);
        }
